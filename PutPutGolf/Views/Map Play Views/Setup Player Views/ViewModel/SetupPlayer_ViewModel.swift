...
//        @AppStorage(AppStorageKeys.savedGames.description) var savedGames: [SavedGame]?
        ...
        init() {
            print("SetupPlayers.\(type(of: self)).\(#function)")
            textFieldDidSubmitSubscriber()
            setupNewPlayerSubscriber()
        }
        
        fileprivate func textFieldDidSubmitSubscriber() {
            $textFieldDidSubmit
        ...
        fileprivate func setupNewPlayerSubscriber() {
            $newPlayers
